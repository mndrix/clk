...
=head2 entry patterns

Any pattern that can uniquely identify a single entry can also be used to
specify an instant.  If an instant is expected and it doesn't match any of the
above formats, it's assumed to be an entry pattern.  The matching entry's
time is used as the instant.
...
The year which includes the current day.

=head2 last year

The year preceding L</this year>.
